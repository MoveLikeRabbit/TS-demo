...
type Chainable<T = {}> = {
    option<K extends PropertyKey, V>(key: K, value: V): Chainable<Omit<T, K> & {
        [P in K]: V
    }>;
    get(): T
}
declare const config: Chainable
const result2 = config.option('foo', 123).option('name', 'type-challenges').option('foo', true).option('bar', { value: 'Hello World' }).get()
